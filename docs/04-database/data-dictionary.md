...
> - **Description**: derived from usage in `app/`, `src/`, `scripts/`, `prisma/seed.ts`. Where usage was not found, marked *(no usage found)* and description is inferred from the name.
...
| biometricEnabled | = | Boolean | N | false | Biometric login preference *(no server usage found; mobile-oriented)* | |
| biometricType | = | String | Y | — | Biometric type *(no usage found)* | |
...
### ClientHiringNeed (`ClientHiringNeed`) — *no app usage*
...
### ProjectCompletionRequest (`ProjectCompletionRequest`) — *no app usage*
...
### ProjectReviewRequest (`ProjectReviewRequest`) — *no app usage*
...
| completionId | = | Int | Y | — | Completion request *(no usage found)* | |
| clientId | = | Int | N | — | | idx |
| professionalId | = | Int | N | — | | idx |
...
### SocketConversationClear (`SocketConversationClear`) — *no app usage*
...
### CallSession (`CallSession`) — *no app usage*
...
### UserNotificationState (`UserNotificationState`) — *no app usage*
...
### BrowserSubscription (`BrowserSubscription`) — *no app usage*
...
### CmsPage (`cms_pages`) — *no app usage*
...
### CmsPageVersion (`cms_page_versions`) — *no app usage*
...
### CmsMedia (`cms_media`) — *no app usage*
...
### WebsitePage (`WebsitePage`) — *no app usage*
...
### LegalPage (`LegalPage`) — *no app usage*
...
### PageConfiguration (`page_configurations`) — *no app usage*
...
### WebsitePageOverride (`website_page_overrides`) — *no app usage*
...
### PageTextOverride (`page_text_overrides`) — *no app usage*
